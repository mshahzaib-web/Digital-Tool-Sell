    ...
    const fetchToolData = async () => {
      try {
        ...
        if (res.data.success) {
          ...
          setImage(t.image || "");
          setCategory(t.category || "Design & AI Tools");
          setPricingType(t.pricingType || "Monthly Subscription");
          setPrice(String(t.price || ""));
          setStockStatus(t.stockStatus || "In Stock");
          setDescription(t.description || "");
          setFeatures(t.features ? t.features.join("\n") : "");
          setDeliveryTime(t.deliveryTime || "Instant");
          setDiscount(String(t.discount || "0"));
          setRating(String(t.rating || "4.8"));
          setWebsiteUrl(t.websiteUrl || "");
          setSellerNotes(t.sellerNotes || "");
        } else {
          setFetchError("Failed to fetch tool details.");
        }
      } catch (err) {
        console.error("Fetch error:", err);
        setFetchError(
          "Error loading tool. It may have been deleted or the server is offline.",
        );
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        pricingType,
        description,
        features, // split by newline in controller
        stockStatus,
        deliveryTime,
        sellerNotes,
        websiteUrl,
      ...
      const res = await toolsService.update(id, payload);
      if (res.data.success) {
      ...
      setFormError(
        err.response?.data?.message ||
          "Error occurred while saving changes. Check admin credentials.",
      );
    } finally {
      setSubmitLoading(false);
    }
    ...
    return (
      <div className="max-w-md mx-auto px-4 py-20 space-y-8">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-black text-slate-855 dark:text-white">
            Admin Authentication Required
          </h1>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            You must be logged in as an administrator to update digital products
            on VaultX.
          </p>
        </div>
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16 space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          Modify the fields below to update{" "}
